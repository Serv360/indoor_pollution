...
```{r}
sort(unique(filtered_data_hh$age))
```
...
filtered_data_hh <- filtered_data_hh[!is.na(filtered_data_hh$age),]
...
filtered_data_hh$age <- factor(filtered_data_hh$age)
...
ggplot(filtered_data_hh, aes(x = as.factor(age), y = edulevel)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + # Rotate x-axis labels for better readability
  scale_x_discrete(breaks = unique(as.character(sort(unique(filtered_data_hh$age))[seq(1, length(unique(filtered_data_hh$age)), by = 5)]))) # Set breaks for every five years
```

```{r}

data_hh$edulevel[data_hh$edulevel > 20] <- NaN
...
literacy_percentage <- data_hh %>% 
  ...
  summarize(literacy_rate = mean(literate, na.rm = TRUE) * 100,
            edulevel_mean = mean(edulevel, na.rm = TRUE),
            sexe = mean(male, na.rm = TRUE) * 100,
            count = n()) %>%
  arrange(desc(count))

literacy_percentage_tot <- data_hh %>% 
  summarize(literacy_rate = mean(literate, na.rm = TRUE) * 100,
            edulevel_mean = mean(edulevel, na.rm = TRUE),
            sexe = mean(male, na.rm = TRUE) * 100,
            count = n()) %>%
...
-    Edulevel ou literate de la personne qui cuisine (proxy
    Spouse/Partner)

-    Edulevel du head du ménage : prend les décisions ?
...
-    Les enfants ont un niveau d'illétrisme plus bas que les heads of
    family (majoritairement des hommes) et que les spouses/partners
    (majoritairement des femmes). Ils ont un edulevel déjà plus haut en
    moyenne à partir de 10-12 ans.
...
```{r}
hh_four_treated <- hh_four %>% filter(treat == 1)
length(unique(hh_four_treated$hhid))
```
...
household_summary <- hh_four %>% filter(treat == 1) %>%
  ...
  summarize(
    ...
    across(starts_with("v_mo_control"), ~first(.)),
    edulevel_spouse_partner = if_else(any(relatehoh_str == "Spouse/partner"), first(edulevel[relatehoh_str == "Spouse/partner"]), NA_real_),
    edulevel_head = if_else(any(relatehoh_str == "Head"), first(edulevel[relatehoh_str == "Head"]), NA_real_)
  ) %>%
...
```{r}
summary(household_summary[, c("mealslowpol_good_2", "n_meals_lastweek", "stovebuilt", "anystove", "goodcond")])
...
x_list = c("literacy_percent", "edulevel_max", "edulevel_mean", "edulevel_spouse_partner", "edulevel_head")
